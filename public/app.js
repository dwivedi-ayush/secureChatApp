...
function createMessageHtml(message) {
  if (message.type === messageTypes.LOGIN) {
    return (
      '<p class="secondary-text text-center mb2">' +
      message.author +
    ...
    if (message.type === messageTypes.LEFT) {
      if (message.flag == 1) {
        return (
          '<div class="message message-left"><div class="message-details flex"><p class="message-author">' +
          message.author +
          '</p><p class="message-date">' +
          message.date +
          '</p></div><img class="clickable message-content" src = "' +
          message.data +
          '"  /></div>'
        );
        ...
        return (
          ...
          '</p><p class="message-date">' +
          message.date +
          '</p></div><img onclick="imageClicked(event)" data-name = "' +
          message.uniqueKey +
          '" class="message-content clickable" src = "' +
          message.imageData +
          '"  /></div>'
        );
      }
      ...
      if (message.flag == 1) {
        return (
          '<div class="message message-right"><div class="message-details flex"><p class="message-author"></p><p class="message-date">' +
          message.date +
          '</p></div><img class="clickable message-content" src = "' +
          message.data +
          '" /></div>'
...
sendBtn2.addEventListener("click", (event) => {
  ...
  messages.map((msg) => {
    if (msg.type == "login") {
      popupform.innerHTML +=
        "<button class='realMsg'>" + msg.author + "</button>";
    }
  });
